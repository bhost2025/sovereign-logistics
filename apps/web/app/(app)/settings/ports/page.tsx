...
const TYPE_LABEL: Record<string, string> = {
  origin: 'Origen', destination: 'Destino', both: 'Ambos',
}
  ...
  return (
    <div className="p-8 max-w-[860px]">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-xl font-extrabold text-[#0a1a3c] tracking-tight">Puertos</h1>
      ...
      <div className="bg-white rounded-xl shadow-[0_1px_20px_rgba(24,28,30,0.06)] p-5 mb-6">
        <div className="text-[10px] font-bold uppercase tracking-widest text-[#8a9aaa] mb-4">+ Agregar Puerto</div>
        <form action={createPort} className="grid grid-cols-4 gap-3">
          ...
          <div>
            <label className="block text-[10px] font-bold text-[#556479] mb-1">País *</label>
          ...
          <div>
            <label className="block text-[10px] font-bold text-[#556479] mb-1">Código</label>
            <input
              name="code"
              placeholder="SHA"
              maxLength={6}
        ...
        <table className="w-full text-xs">
          <thead>
            <tr className="border-b border-[#f0f2f5]">
              {['Puerto', 'País', 'Código', 'Tipo', 'Estado', ''].map(h => (
                    ...
                    <div className="flex items-center gap-3">
                      <form action={togglePort.bind(null, port.id, !port.active)}>
                        <button type="submit" className="text-[10px] font-bold text-[#4A6FA5] hover:text-[#0a1a3c]">
                          {port.active ? 'Desactivar' : 'Activar'}
                        </button>
                      </form>
            ...
            {(!ports || ports.length === 0) && (
              <tr>
                <td colSpan={6} className="px-5 py-8 text-center text-[11px] text-[#8a9aaa]">
                  No hay puertos registrados. Agrega el primero arriba.
                </td>
              </tr>
            )}
          </tbody>
